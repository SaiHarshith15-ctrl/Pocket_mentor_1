 ...
 * heatmap (from GET /api/profile/activity), subject mastery, stats, and
 * achievements. All server-driven.
 */
...
export default function Profile() {
  ...
  // Build a lookup by date string for the heatmap grid.
  const activityMap = {};
  activity.forEach((a) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <div className="card text-center">
          <p className="text-3xl font-bold text-brand-light">{profile.overallMastery}%</p>
          <p className="text-slate-400 text-sm">Overall Mastery</p>
        </div>
        <div className="card text-center">
          <p className="text-3xl font-bold">🔥 {profile.currentStreak}</p>
          <p className="text-slate-400 text-sm">Day Streak (best {profile.longestStreak})</p>
        </div>
        <div className="card text-center">
          <p className="text-3xl font-bold">{profile.stats.topicsMastered}</p>
        ...
        <div className="card">
          ...
          <ul className="text-sm text-slate-300 space-y-1.5">
            <li className="flex justify-between">
              ...
              <span className="font-medium">{profile.stats.questionsAnswered}</span>
            </li>
            <li className="flex justify-between">
              <span>Flashcards reviewed</span>
              <span className="font-medium">{profile.stats.flashcardsReviewed}</span>
            </li>
            <li className="flex justify-between">
              <span>Topics mastered</span>
              <span className="font-medium">{profile.stats.topicsMastered}</span>
            </li>
            <li className="flex justify-between">
              <span>Topics rescued</span>
              <span className="font-medium">{profile.stats.topicsRescued}</span>
            </li>
          </ul>
        </div>
        ...
        <div className="card">
          ...
          <div className="space-y-2">
            ...
            {profile.subjectMastery.length === 0 && (
              <p className="text-slate-500 text-sm">No subjects tracked yet.</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="flex flex-wrap gap-2">
          {profile.achievements.length === 0 && (
            <p className="text-slate-500 text-sm">None unlocked yet — keep studying!</p>
          )}
